...
class TraceResponse(BaseModel):
    ...
    best_match_title: str | None      # so UI shows doc name in comparison card
    best_match_content: str | None    # so SourceComparisonCard renders real source text
...
class FeedbackRequest(BaseModel):
    claim_text: str
    confirmed: bool   # True = match was correct, False = wrong match


@router.post("/trace-claim", response_model=TraceResponse)
async def trace_claim(req: TraceRequest):
    """
    Given any claim, find its most likely primary source and compute drift.
    Returns full doc content so the frontend can render a real side-by-side.
    """
...
async def test_your_own(req: TestPairRequest):
    """
    Sandbox: judge pastes ANY source + claim pair. Writes into isolated
    live_test dataset so curated main graph is never polluted.
    """
...
async def submit_feedback(req: FeedbackRequest):
    """
    Judge confirms or rejects a match — triggers improve() so Cognee
    sharpens its weights. This is the live demo of the improve() op.
    """
    from app.memory.cognee_client import improve as run_improve, remember_live_test
    # Write the feedback signal into Cognee as a training note
    label = "CONFIRMED MATCH" if req.confirmed else "INCORRECT MATCH"
    await remember_live_test(
        "FEEDBACK",
        f"[{label}] Claim: {req.claim_text[:200]}"
    )
    # Now run improve() — Cognee enriches/re-weights the graph
    await run_improve()
    return {
        "status": "improved",
        "message": "Cognee memory updated — future recalls will reflect this feedback"
    }


@router.post("/improve")
async def trigger_improve():
    """Manually trigger memify."""
    await improve()
...
async def forget_resolved(dataset: str = "live_test"):
    """
    Mark a dataset as resolved and prune it from Cognee's graph.
    Demo: call this after a claim is fully traced and archived.
    """
